...
map<int, map<int, double>> GenerateBarycentricCoord(MatrixXd V_f, MatrixXi T_f, MatrixXd V)
{
    ...
    for (int n = 0; n < T_f.rows(); n++)
    {
        ...
        int i_max = floor(max(0) + 0.5);
        int i_min = floor(min(0) + 0.5);
        int j_max = floor(max(1) + 0.5);
        int j_min = floor(min(1) + 0.5);
        int k_max = floor(max(2) + 0.5);
        int k_min = floor(min(2) + 0.5);
    ...
    {
        cout << "Check if all the vertices are in frame model!!" << endl;
        exit(100);
    }
    return baryCoords;
}
...
bool CalculateScalingWeights(MatrixXd &C, MatrixXd &V, MatrixXi &T, MatrixXd &W)
{
    ...
    for (int n = 0; n < C.rows(); n++)
    {
        ...
        for (int i : grid[key])
        {
            if (fabs(C(n, 0) - V(i, 0)) > 0.01)
                continue;
            if (fabs(C(n, 1) - V(i, 1)) > 0.01)
                continue;
            if (fabs(C(n, 2) - V(i, 2)) > 0.01)
                continue;
            b(n) = i;
            bc(n, n) = 1;
            break;
...
{
    VectorXi b;
    b.resize(C.rows() - 3 + eyeLV.size() + eyeRV.size());
